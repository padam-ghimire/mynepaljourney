...
class AgencySocialLogin
{
    ...
    private function configuration()
    {
        ...
        $configuration = gs()->agency_socialite_credentials->$provider;
        Config::set('services.' . $provider, [
            'client_id'     => $configuration->client_id,
            'client_secret' => $configuration->client_secret,
            'redirect'      => route('agency.social.login.callback', $provider),
        ]);
    ...
    public function login()
    {
        ...
        $userData = Agency::where('username', $username)->first();

        if (!$userData) {
            $emailExists = Agency::where('email', $user->email)->exists();
            if ($emailExists) {
        ...
        auth()->guard('agency')->login($userData);
    ...
    private function createUser($user, $provider)
    {
        ...
        if ($user->avatar) {
            ...
            file_put_contents(getFilePath('agencyProfile') . '/' . $fileName, file_get_contents($this->provider != 'facebook' ? $user->avatar : $user->avatar . '&access_token=' . $user->token));
        ...
        $adminNotification->agency_id = $newUser->id;
        $adminNotification->title = 'New member registered';
        $adminNotification->click_url = urlPath('admin.agencies.detail', $newUser->id);
