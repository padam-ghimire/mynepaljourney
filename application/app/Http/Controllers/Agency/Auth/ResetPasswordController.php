...
class ResetPasswordController extends Controller
{
    ...
    */
    ...
    public function showResetForm(Request $request, $token = null)
    {
        ...
        if (AgencyPasswordReset::where('token', $token)->where('email', $email)->count() != 1) {
            $notify[] = ['error', 'Invalid token'];
            return to_route('agency.password.request')->withNotify($notify);
        }
        return view($this->activeTemplate . 'agency.auth.passwords.reset')->with(
    ...
    public function reset(Request $request)
    {
        ...
        $reset = AgencyPasswordReset::where('token', $request->token)->orderBy('created_at', 'desc')->first();
        if (!$reset) {
            $notify[] = ['error', 'Invalid verification code'];
            return to_route('agency.login')->withNotify($notify);
        }

        $user = Agency::where('email', $reset->email)->first();
