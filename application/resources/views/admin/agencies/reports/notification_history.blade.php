...
@section('panel')
    <div class="row">
        @include('admin.components.tabs.notification')
        <div class="col-lg-12">
                                ...
                                @forelse($logs as $log)
                                    <tr>
                                        <td>
                                            @if ($log->agency)
                                                <a
                                                    href="{{ route('admin.agencies.detail', $log->agency_id) }}">{{ $log->agency->fullname ?? $log->agency->username }}</a>
                                            @else
                                        ...
                                        <td>
                                            <button title="@lang('Details')" class="btn btn-sm btn--primary notifyDetail"
                                                ...
                                                @if ($log->notification_type == 'email') data-message="{{ route('admin.agency.report.email.details', $log->id) }}" @else
...
@push('breadcrumb-plugins')
    @if (isset($user))
        <a href="{{ route('admin.agencies.notification.single', $user->id) }}" class="btn btn--primary btn-sm"><i
